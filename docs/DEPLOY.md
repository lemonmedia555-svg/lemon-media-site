...
Скрипт `deploy.sh` запускается только с паролем root из локальной переменной `LEMON_VPS_PASS`; значения по умолчанию в проекте больше нет. Для продакшена:

1. Сменить старый пароль сервера, если он когда-либо попадал в репозиторий.
2. Перед временным запуском через пароль задать `LEMON_VPS_PASS` только в локальном окружении.
3. Для постоянного деплоя сгенерировать SSH-ключ и положить публичную часть в `~/.ssh/authorized_keys` на VPS.
4. Переключить скрипт на `ssh -i ~/.ssh/lemon-media-key` и отключить вход root по паролю.
